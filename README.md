...
tests/fm-stall-check.test.sh              # stall detector: finished-but-not-advanced, unblocked/date-gated queued, unlanded-work sweep with secondmate/scout/local-only/pr-parked/fork-remote/mid-rebase exemptions, idle-pane stalls, idle-advisor detection for working/terminal statuses with busy/active-child/pr-parked-child/needs-decision/blocked exclusions, unrelayed child-escalation sweep with recent/busy-pane exclusions and advisor-state independence, terminal-child handling, pr-ready skip, done-archive fallback, and guard pointer
