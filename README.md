...
| `fm-error-harvest.py`    | Read Claude Code transcripts to group recurring tool errors, denials, and hook failures by session; output is untrusted and may contain sensitive text |
| `fm-git-audit.sh`        | Read-only report of unlanded work, worktrees, and stashes; fetches remotes unless `--no-fetch`, and `--disk` adds sizes |
...
tests/fm-git-audit.test.sh                # git audit remote-copy classification, refresh/prune behavior, detached worktree coverage, and missing-main handling
tests/fm-error-harvest.test.sh             # recurring-failure transcript harvesting: grouping, filtering, untrusted-text hygiene, unreadable input, and report/JSON exit semantics
