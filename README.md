...
tests/fm-teardown.test.sh                 # fm-teardown.sh safety and reminder checks: local-only fork-remote allow, truly-unpushed refuse, merged-to-main allow, merged-PR head-sha evidence allow with open/errored/404/absent-gh-axi/dirty-worktree/other-head/no-head-sha refusals, no-mistakes regression, tasks-axi reminder, --force override
