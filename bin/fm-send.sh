...
# LOCAL FEATURE re-grafted over upstream during the 2026-08-21 sync. Upstream has no
# equivalent: it ships the reporting contract only in the fm-brief.sh scaffold, so follow-up
# work sent straight into a live lane arrives without it. On 2026-08-18 lanes ran silent for
# hours because of exactly that - a lane that never writes status is invisible to every
# status-based check (see fm-stall-check.sh check_silent_lanes, also local-only).
#
# Attach the contract to the FIRST steer a lane receives and no later: once its status file
# exists it has demonstrably learned to report, so every later steer stays a clean one-liner.
# Set FM_SEND_NO_CONTRACT=1 to suppress. Resolved through upstream's TARGET_META rather than
# by parsing the raw argument, so it no longer needs a TARGET_ARG global.
status_contract_suffix() {
  local id status
  [ -n "${FM_SEND_NO_CONTRACT:-}" ] && return 0
  [ -n "${TARGET_META:-}" ] || return 0
  id=$(fm_send_id_from_meta "$TARGET_META") || return 0
  [ -n "$id" ] || return 0
...
if [ "${1:-}" = "--key" ]; then
  ...
  # Appended AFTER the ledger copy so the recorded answer stays plain.
  MESSAGE="$MESSAGE$(status_contract_suffix)"
