...
# LOCAL ADDITION, re-applied over the upstream guard during the 2026-08-21 sync.
# Upstream has no stall-detector concept and bin/fm-stall-check.sh does not exist there,
# so taking upstream's guard wholesale silently drops this alarm. It is a third independent
# hazard alongside the watcher banner and queued wakes: dormant work that no wake will ever
# surface, because nothing changed to wake anyone. Guarded on the script existing so a
# future upstream sync that removes it degrades quietly instead of erroring.
