...
render_row() {
  local meta=$1 id win kind status_file receipt status_age meta_age age state reason class pr
  ...
  state=idle; reason='Awaiting the next durable update.'; class=idle
  ...
  if terminal_receipt "$receipt" || [ -n "$pr" ]; then
    ...
    state=working; class=working; reason='Busy footer observed in the pane.'
