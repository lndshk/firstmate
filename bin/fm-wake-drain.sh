...
cleanup() {
  ...
  # Restore the queue when a drain fails, THEN clean up the temp. Upstream defines
  # fm_wake_restore_queue() in fm-wake-lib.sh but never calls it, so taking its cleanup
  # wholesale would silently lose every drained wake on any failure - the exact class of
  # bug the wake queue exists to prevent. Both halves are kept deliberately.
