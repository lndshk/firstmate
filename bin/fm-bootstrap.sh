...
if command -v tmux >/dev/null 2>&1 && [ ! -f "$FM_HOME/.fm-secondmate-home" ] && [ -x "$SCRIPT_DIR/fm-artifact-supervisor-service.sh" ]; then
  "$SCRIPT_DIR/fm-artifact-supervisor-service.sh" start >/dev/null 2>&1 || true
fi
exit 0
