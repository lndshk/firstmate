...
# LOCAL-ONLY, re-grafted over upstream during the 2026-08-21 sync. bin/fm-supervisor.sh is the
# always-on deterministic observer and does not exist upstream at all - upstream's
# "supervision" scripts are watcher-repair instructions, a different concept. Without this
# block nothing starts the observer at session start and its snapshot and board go stale
# silently. Secondmates keep their parent-supervised lifecycle, so they skip it.
